...
def test_background_worker_hides_known_tracing_warnings_in_normal_mode(monkeypatch):
    root_logger = logging.getLogger()
    monkeypatch.setattr(root_logger, "isEnabledFor", lambda level: False)

    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        with bgr_worker_module._background_removal_warning_scope():
            warnings.warn(
                "torch.meshgrid: in an upcoming release, it will be required to pass the indexing argument.",
                UserWarning,
            )
            warnings.warn("trace-only detail", torch.jit.TracerWarning)

    assert caught == []


def test_background_worker_preserves_tracing_warnings_in_debug_mode(monkeypatch):
    root_logger = logging.getLogger()
    monkeypatch.setattr(root_logger, "isEnabledFor", lambda level: True)

    with pytest.warns(torch.jit.TracerWarning, match="trace-only detail"):
        with bgr_worker_module._background_removal_warning_scope():
            warnings.warn("trace-only detail", torch.jit.TracerWarning)
