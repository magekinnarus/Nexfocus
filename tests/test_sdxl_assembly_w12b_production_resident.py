...
def test_telemetry_envelope_success(monkeypatch, capsys, caplog):
    task = TaskState()
    task.base_model_name = "test_model.safetensors"
    task.sdxl_assembly_posture = "streaming"
    task_dict = {"task_seed": 12345}
    ...
    monkeypatch.setattr("backend.sdxl_assembly.director.SDXLAssemblyDirector.select_assembly", lambda r, **_kwargs: mock_assembly)
    ...
    assert "[Nex] Inference:" in captured.out
    assert "[Nex] Inference complete in" in captured.out
    assert "[SDXL RUN BEGIN]" in caplog.text
    assert "[SDXL RUN END]" in caplog.text
    assert "SUCCESS" in caplog.text
    assert "Correlation ID:" in caplog.text

def test_telemetry_envelope_failure(monkeypatch, capsys, caplog):
    task = TaskState()
    task.base_model_name = "test_model.safetensors"
    task.sdxl_assembly_posture = "streaming"
    task_dict = {"task_seed": 12345}
    ...
    captured = capsys.readouterr()
    assert "[Nex] Inference failed after" in captured.out
    assert "Route: txt2img_assembly" in captured.out
    assert "[SDXL RUN BEGIN]" in caplog.text
    assert "[SDXL RUN END]" in caplog.text
    assert "FAILURE" in caplog.text
    assert "Error: Mock error execution failure" in captured.out

def test_telemetry_envelope_interrupt(monkeypatch, capsys, caplog):
    task = TaskState()
    task.base_model_name = "test_model.safetensors"
    task.sdxl_assembly_posture = "streaming"
    task_dict = {"task_seed": 12345}
    ...
    captured = capsys.readouterr()
    assert "[Nex] Inference interrupted after" in captured.out
    assert "[SDXL RUN BEGIN]" in caplog.text
    assert "[SDXL RUN END]" in caplog.text
